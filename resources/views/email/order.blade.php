...
<body style="font-family: Arial, Helvetica, san-serif; font-size:16px;">
    ...
    <h2>Your Order Is: #{{$mailData['order']->id}}</h2>
    @else
    ...
    <table cellpadding="3" cellspacing="3" border="0" width="700">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>PHP{{number_format($mailData['order']->subtotal,2)}}</td>
            </tr>

            {{-- <tr>
                <th colspan="3" align="right">Discount:{{ (!empty($mailData['order']->coupon_code)) ? '('.$mailData['order']->coupon_code.')': '' }}</th>
                <td>PHP{{number_format($mailData['order']->discount,2)}}</td>
            </tr> --}}
            
            <tr>
                <th colspan="3" align="right">Shipping:</th>
                <td>PHP{{number_format($mailData['order']->shipping,2)}}</td>
            </tr>
            <tr>
                <th colspan="3" align="right">Grand Total:</th>
                <td>PHP{{number_format($mailData['order']->grand_total,2)}}</td>
            </tr>
        </tbody>
    </table>		
</body>
